...
import { suburbKey } from '../lib/raw.js'
import {
  ReaCaptureSchema,
  excludedByKeyword,
  flattenCapture,
...
import { scoreListing, unenrichedBlock } from '../lib/score.js'

export async function main(argv: string[]): Promise<void> {
  const criteria = await readJsonFile(dataPath('config', 'criteria.json'), CriteriaSchema)
  const capture = ReaCaptureSchema.parse(JSON.parse(readFileSync(argv[0], 'utf8')))
