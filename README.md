...
| `REALESTATE_MCP_ROUTER` | `valhalla` | **Road modes only — `walk` and `drive`.** `valhalla` (no key), `ors` (needs `ORS_API_KEY`), or `google` (needs `GOOGLE_MAPS_API_KEY`). Transit never consults it: that goes to TfNSW when `TFNSW_API_KEY` is set and to Google otherwise, so `tfnsw` is not a value here. Anything unrecognised falls through to Valhalla, which needs no key and would otherwise fail silently — the server writes a `configuration error` line to stderr instead |
